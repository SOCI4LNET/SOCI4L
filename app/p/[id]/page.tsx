                        ...
                        <div className="flex flex-col items-center text-center mb-6 relative w-full pt-2">
                            ...
                            <div className="absolute top-0 right-0 flex items-center gap-2">
                                {profileAddressForFollow && !isOwnProfile && !profile?.isBanned && (
                                    <div className="hidden sm:flex gap-2 [&_button]:px-[14px] [&_button]:h-8 [&_button]:rounded-md [&_button]:bg-foreground/5 [&_button]:border [&_button]:border-foreground/10 [&_button:hover]:bg-foreground/10 [&_button]:text-foreground [&_button]:font-medium [&_button]:backdrop-blur-md [&_button]:transition-all [&_button:active]:scale-95 [&_svg]:h-[14px] [&_svg]:w-[14px] [&_button]:gap-1.5 [&_button]:text-xs">
                                        <FollowToggle
                                ...
                                {resolvedAddress && isValidAddress(resolvedAddress) && !profile?.isBanned && (
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <button className="h-8 w-8 shrink-0 rounded-md bg-foreground/5 border border-foreground/10 flex items-center justify-center text-foreground/70 hover:text-foreground hover:bg-foreground/10 transition-all active:scale-95 backdrop-blur-md [&_svg]:stroke-[1.5]">
                ...
                profileAddressForFollow && !profile?.isBanned && resolvedAddress && isValidAddress(resolvedAddress) && (
                    <div className="fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom)+0.5rem)] z-50 md:hidden px-3">
                        <div className="rounded-2xl border border-border/80 bg-background/95 p-2 shadow-2xl backdrop-blur supports-[backdrop-filter]:bg-background/80">
                            <div className={isOwnProfile ? "flex w-full" : "grid grid-cols-3 gap-2"}>
                                <Button
                                    variant="default"
                                    size="sm"
