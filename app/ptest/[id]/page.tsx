...
import { ExternalLink, Linkedin, Github, Globe, MessageCircle, Send, Mail, QrCode, Link2, Activity, Copy, ArrowRight, Heart, Share2, Instagram, Youtube, Sparkles, ShieldAlert, Layers, UserX, CheckCircle, MoreVertical, Ban, Puzzle, Code2, ChevronDown, Wallet } from 'lucide-react'
                                    ...
                                    <div className="flex items-start gap-4 min-w-0 flex-1">
                                        ...
                                        <div className="min-w-0 space-y-2 flex-1">
                                            ...
                                            <div className="space-y-1">
                                                {!profile?.isBanned && (isProMember || (score && score.total > 0)) && (
                                                    <div className="flex items-center gap-2 text-xs">
                                                        {isProMember && (
                                                            <span className="inline-flex items-center rounded-full px-2 py-0.5 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white font-semibold">
                                                                Pro
                                                            </span>
                                                        )}
                                                        {score && score.total > 0 && (
                                                            <span className="text-muted-foreground font-medium">
                                                                {isProMember ? `· ${score.tierLabel}` : score.tierLabel}
                                                            </span>
                                                        )}
                                                    </div>
                                                )}
                                                {!profile?.isBanned && (
                                                    (() => {
                                                        const roleStatusItems = [
                                                            ...
                                                            profile?.role === 'ADMIN' ? 'Team' : null,
                                                            profile?.role === 'BUILDER' ? 'Builder' : null,
                                                            isClaimed && !isPrivate ? 'Claimed' : null,
                                                        ].filter(Boolean)
                                                        if (roleStatusItems.length === 0) return null
                                                        return (
                                                            <p className="text-xs text-muted-foreground/80">
                                                                {roleStatusItems.join(' · ')}
                                                            </p>
                                                        )
                                                    })()
                                                )}
                                                {profile?.isBanned && (
                                                    <Badge variant="destructive" className="text-[11px] px-2 py-0 font-normal flex items-center gap-1 w-fit">
                                                    ...
                                                    <div key={link.id || link.url} className="relative inline-block">
                                                        ...
                                                        {link.verified && (
                                                            <div className="absolute -top-1 -right-1 pointer-events-none bg-background rounded-full p-[1px] ring-1 ring-border/20 shadow-sm z-10">
                                                                <CheckCircle className="h-2 w-2 text-blue-500/70 fill-blue-500/10" />
                                                            </div>
                                                        )}
                                                    </div>
                                                )
                            ...
                            <div className="space-y-6 w-full">
                              ...
                              Views · Clicks (7d)
                                        ...
                                        if (sectionId === 'assets') {
                                            ...
                                            return (
                                                <Card key="assets" className={`${getThemeCardClasses(effectiveAppearanceConfig.theme, 'assets')} ${gridColSpan} w-full overflow-hidden relative pb-10`}>
                                                    <div className="px-6 py-5 border-b border-border/50 flex items-center justify-between">
                                                        <div className="flex items-center gap-2.5">
                                                            <div className="p-1.5 rounded-lg bg-muted text-muted-foreground">
                                                                <Wallet className="h-4 w-4" />
                                                            </div>
                                                            <h3 className={getThemeTextClasses(effectiveAppearanceConfig.theme, 'title')}>Assets</h3>
                                                        </div>
                                                        <span className="text-xs font-mono text-muted-foreground">{allAssets.length} Total</span>
                                                    </div>

                                                    <div className="p-2">
                                                        <div className="flex flex-col">
                                                            {visibleAssets.map((asset, idx) => (
                                                                <div key={idx} className="group flex items-center justify-between p-3 sm:p-4 rounded-2xl hover:bg-muted/50 transition-colors">
                                                    ...
                                                    {!expandedAssets && hasMoreAssets && (
                                                        <div className="absolute bottom-0 left-0 w-full h-32 bg-gradient-to-t from-background via-background/80 to-transparent flex items-end justify-center pb-5 pointer-events-none">
                                                            ...
                                                            <button
                                                                ...
                                                                className="pointer-events-auto relative z-10 flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors bg-muted/50 hover:bg-muted backdrop-blur-md px-5 py-2.5 rounded-full border shadow-sm active:scale-95 duration-200"
                                                            >
                                                                View All Assets
                                                    ...
                                                    {expandedAssets && hasMoreAssets && (
                                                        <div className="p-4 flex justify-center border-t">
                                                            <button
                                                                ...
                                                                className="flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors py-2 active:scale-95 duration-200"
                                                            >
                                                                Show Less
