  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/90 backdrop-blur supports-[backdrop-filter]:bg-background/70">
        <div className={cn('w-full', PAGE_GUTTER, 'py-2.5 md:py-4')}>
          <div className={cn('w-full', CONTENT_MAX_WIDTH, 'mx-auto')}>
            <div className="flex items-center justify-between">
              <Link href="/" className="flex items-center">
                <Soci4LLogo variant="combination" width={108} height={19} className="md:w-[120px] md:h-[21px]" />
              </Link>
              <div className="flex items-center gap-2">
                {!mounted ? (
                  ...
                  <WalletConnectButtons variant="default" size="sm" className="h-9 px-3 md:h-10 md:px-4" />
                )}
              </div>
            </div>
          </div>
