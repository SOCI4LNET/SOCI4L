...
// Wordmark SVG paths extracted from /public/logos/wordmark.svg
...
export default function SiteFooter({ className }: SiteFooterProps = {}) {
    ...
    return (
        <footer className={cn('w-full bg-foreground text-background overflow-hidden relative', className)}>

            {/* Main content */}
            <div className="relative z-10 max-w-7xl mx-auto px-8 pt-16 pb-10">

                {/* Top grid: Logo | Product | Resources | Stay Updated */}
                <div className="grid grid-cols-1 gap-10 md:grid-cols-4 mb-16">

                    {/* Logo + tagline */}
                    <div className="space-y-3">
                        <Soci4LLogo variant="combination" width={110} height={22} className="invert-0 dark:invert" />
                        <p className="text-sm text-background/50 leading-relaxed max-w-[180px]">
                    ...
                    {/* Stay updated */}
                    <div className="space-y-4">
                        <h3 className="text-sm font-semibold text-background">Stay updated</h3>
                        <form onSubmit={handleEmailSubmit} className="flex gap-2">
                            <Input
                                ...
                                className="flex-1 bg-background/10 border-background/20 text-background placeholder:text-background/40 focus-visible:ring-background/30 h-9 text-sm"
                            ...
                            <Button
                                ...
                                size="sm"
                                disabled={isSubmitting}
                                className="bg-background text-foreground hover:bg-background/90 font-medium h-9 px-4 shrink-0"
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-6 border-t border-background/10">
                    <p className="text-sm text-background/40">
                        © {currentYear} SOCI4L. All rights reserved.
                    </p>
                    <div className="flex items-center gap-1">
                        <a
                            href="https://x.com/soci4lnet"
                            target="_blank"
                            rel="noopener"
                            className="p-2 rounded-md text-background/40 hover:text-background hover:bg-background/10 transition-colors"
                            aria-label="Follow us on X"
                        >
                            <XIcon className="h-4 w-4" />
                        </a>
                        <a
                            href="https://github.com/SOCI4LNET"
                            target="_blank"
                            rel="noopener"
                            className="p-2 rounded-md text-background/40 hover:text-background hover:bg-background/10 transition-colors"
                        ...
                        <div className="ml-1 pl-3 border-l border-background/10 h-6 flex items-center">
                            <ModeToggle />
                        </div>
                    </div>
                </div>
            ...
            {/* Large SOCI4L watermark — bottom, cropped, gradient fade */}
            <div className="relative z-0 w-full overflow-hidden" style={{ height: '160px', marginTop: '-24px' }}>
                {/* Gradient overlay: fades top */}
                <div className="absolute inset-0 bg-gradient-to-b from-foreground via-foreground/60 to-foreground/0 z-10 pointer-events-none" />
                <div
                    className="absolute inset-x-0 bottom-0 text-background/[0.06]"
                    style={{ height: '220px' }}
                >
                    <WordmarkPaths />
                </div>
