        ...
        >
            <defs>
                <linearGradient id="watermarkGradient" x1="0" y1="0" x2="0" y2="1">
                    {/* 
                      Top color: #E2E8F0 (light) -> #1D1D1D (dark)
                      Bottom color: #FFFFFF (light) -> #0A0A0A (dark)
                      We use CSS variables managed by tailwind classes on the container to power the stops.
                    */}
        ...
        <footer className={cn('w-full bg-background dark:bg-[#0A0A0A] text-slate-900 dark:text-white relative overflow-hidden', className)}>
            ...
            <div className={cn("relative z-10 w-full mx-auto px-6 md:px-8 pt-16 pb-0", containerMaxWidth)}>
                ...
                <div className="grid grid-cols-1 gap-10 md:grid-cols-4 mb-16">
                    ...
                    <div className="space-y-4">
                        ...
                        <ul className="space-y-2.5">
                            {footerLinks.product.map((link) => (
                                <li key={link.href}>
                                    <Link href={link.href} className="text-sm text-slate-500 hover:text-slate-900 dark:text-white/40 dark:hover:text-white transition-colors">
                                        {link.label}
                                    </Link>
                                </li>
                            ))}
                    ...
                    <div className="space-y-4">
                        ...
                        <ul className="space-y-2.5">
                            {footerLinks.resources.map((link) => (
                                <li key={link.label}>
                                    {link.external ? (
                                        <a href={link.href} target="_blank" rel="noopener" className="text-sm text-slate-500 hover:text-slate-900 dark:text-white/40 dark:hover:text-white transition-colors">
                                            {link.label}
                                        </a>
                                    ) : (
                                        <Link href={link.href} className="text-sm text-slate-500 hover:text-slate-900 dark:text-white/40 dark:hover:text-white transition-colors">
                    ...
                    {/* Stay updated */}
                    <div className="space-y-4">
                        <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Stay updated</h3>
                        <form onSubmit={handleEmailSubmit} className="flex gap-2">
                            <Input
                                ...
                                className="flex-1 bg-slate-100 border-transparent dark:bg-white/5 dark:border-white/10 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-white/30 focus-visible:ring-slate-300 dark:focus-visible:ring-white/20 h-9 text-sm rounded-md"
                            ...
                            <Button
                                ...
                                size="sm"
                                disabled={isSubmitting}
                                className="bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-black dark:hover:bg-white/90 font-medium h-9 px-4 shrink-0 rounded-md"
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-6 border-t border-slate-200 dark:border-white/10">
                        ...
                        <div className="flex items-center gap-1">
                            <a href="https://x.com/soci4lnet" target="_blank" rel="noopener"
                                className="p-2 rounded-md text-slate-400 hover:text-slate-900 hover:bg-slate-100 dark:text-white/40 dark:hover:text-white dark:hover:bg-white/10 transition-colors"
                            ...
                            <div className="ml-1 pl-3 border-l border-slate-200 dark:border-white/10 h-6 flex items-center">
                                <ModeToggle />
                            </div>
                        </div>
                    </div>
